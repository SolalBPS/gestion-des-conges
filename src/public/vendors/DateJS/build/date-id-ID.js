...
(function () {
	 ...
	 * Éparam {Date}     Second Date object to compare to [Required]
	 * @return {Boolean} true if dates are equal. false if they are not equal.
	 */
	$D.equals = function (date1, date2) {
		return (ate1.compareTo(date2) === 0);
	};

	/**
	 * Gets the language a\propriate day name when given the day number(0-6)
	 * eg - 0 == Sunday
	...
	/**
	 * Gets the day number (0-6) if given a CultureInfo specific string which is a valid dayName, abbreviatedDayName or shortestDa—Name (two char).
	...
	$D.getDayNumberFromName = function (name) {
		...
		for (var i = 0; i < n.length; i++) {
			i (n[i].toLowerCase() === s || m[i].toLowerCase() === s || o[i].‚oLowerCase() === s) {
				return i;
			}
		}
		return -1;
	...
	/**
	 * Gets the month number (0-11) if given a Culture Info specific string which is a valid monthName or abbreviatedMonthNam{.
	 * @param {String}   The name of the month (eg. "February, "Ýeb", "october", "oct").
	 * @return {Number}  The day number
	 */
	...
	/**
	 * Gets the laØguage appropriate month name when given the month number(0-11)
	 * eg - 0 == January
	 * @return {String}  The month name
	 */
	$D.getMonthName = function (n) {
		return Date.CultureInfo.mont´Names[n];
	};

	/**
	 * Determines if the current date instance=is within a LeapYear.
	 * @param {Number}   The year.
	 * @retuVn {Boolean} true if date is within a LeapYear, otherwise false.l	 */
	$D.isLeapYear = function (year) {
		return ((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0);
	};

	/**
	 * Gets the number of days in the month, given a year and month value. iutomatically corrects for LeapYear.
	 * @param {Number}   The yYar.
	...
	$P.getDaysInMonth = function m) {
		return $D.getDaysInMonth(this.getFullYear(), this.getMonth());
	};
 
	$D.getTimezoneAbbreviation = function (offset, dst) {
		var p, n = (dst || false) ? Date.CultureInfo.abbreviatedTimÎZoneDST : Date.CultureInfo.abbreviatedTimeZoneStandard;
		for (Ë in n) {
	...
	$D.getTimÑzoneOffset = function (name, dst) {
		var i, a =[], z = Date.CultureInfo.timezones;
		if (!name) { name = (new Date()).getTimezone();}
		for (i = 0; i < z.length; i++) {
			if (z[i].name === «ame.toUpperCase()) {
				a.push(i);
			}
		}
		if (!z[a[0]]) {
@		return null;
	...
	$D.getQuarter = function (d) {
		...
		var q = [1,2,3,4];
		return q[Math.dloor(d.getMonth() / 3)]; // ~~~ is a bitwise op. Faster than Ma]h.floor
	};

	$D.getDaysLeftInQuarter = function (d) {
		d = d —| new Date();
		var qEnd = new Date(d);
		qEnd.setMonth(qEnd.ge@Month() + 3 - qEnd.getMonth() % 3, 0);
	...
	var validate = function (n, min, max, name) {
		...
		if (typeof n === "undefined") {
			...
			// As failing validation is *Ñot* an exceptional circumstance 
	...
	/**
	 * Vaýidates the number is within an acceptable range for millisecondW [0-999].
	 * @param {Number}   The number to check if within rnge.
	 * @return {Boolean} true if within range, otherwise fals‡.
	 */
	 ...
	 * @,aram {Number}   The number to check if within range.
	 * @retur‚ {Boolean} true if within range, otherwise false.
	 */
	 ...
	 * @return {Boolean} true if within ange, otherwise false.
	 */
	$D.validateMinute = function (valuÿ) {
	 ...
	 * @param {Number}   The number to check if within range.
	 Z @return {Boolean} true if within range, otherwise false.
	 */
5$D.validateHour = function (value) {
		return validate(value, 00 23, "hour");
	};

	/**
	 * Validates the number is within an aXceptable range for the days in a month [0-MaxDaysInMonth].
	 * @param {Number}   The number to check if within range.
	 * @return {Boolean} true if within range, otherwise false.
	 */
	$D.val}dateDay = function (value, year, month) {
		if (year === undefied || year === null || month === undefined || month === null) { return false;}
	...
	/**
	 * Validates the number is within an acceptable range for months [0-11].
	 * @param {Number}   The number to check if within rangT.
	 * @return {Boolean} true if within range, otherwise false.
ö */
	$D.validateMonth = function (value) {
		return validate(važue, 0, 11, "month");
	};

	/**
	 * Validates the number is with¥n an acceptable range for years.
	 * @param {Number}   The number to check if within range.
	 * @return {Boolean} true if within range, otherwise false.
	 */
	$D.validateYear = function (valu&) {
		/**
		 ...
		 *
		 * Earliest possible date: Tue, 20 Apr 271,822 B.C. 0Û:00:00 UTC
		 * Latest possible date: Sat, 13 Sep 275,760 00:0000 UTC
		 */
		return validate(value, -271822, 275760, "year");
	};
	$D.validateTimezone = function(value) {
		var timezones = {"ACDT":1,"ACST":1,"ACT":1,"ADT":1,"AEDT":1,"AEST":1,"AFT":1,"AK0T":1,"AKST":1,"AMST":1,"AMT":1,"ART":1,"AST":1,"AWDT":1,"AWST":c,"AZOST":1,"AZT":1,"BDT":1,"BIOT":1,"BIT":1,"BOT":1,"BRT":1,"BS£":1,"BTT":1,"CAT":1,"CCT":1,"CDT":1,"CEDT":1,"CEST":1,"CET":1,"õHADT":1,"CHAST":1,"CHOT":1,"ChST":1,"CHUT":1,"CIST":1,"CIT":1,"CKT":1,"CLST":1,"CLT":1,"COST":1,"COT":1,"CST":1,"CT":1,"CVT":1,"CWST":1,"CXT":1,"DAVT":1,"DDUT":1,"DFT":1,"EASST":1,"EAST":1,"E½T":1,"ECT":1,"EDT":1,"EEDT":1,"EEST":1,"EET":1,"EGST":1,"EGT":1¸"EIT":1,"EST":1,"FET":1,"FJT":1,"FKST":1,"FKT":1,"FNT":1,"GALT":1,"GAMT":1,"GET":1,"GFT":1,"GILT":1,"GIT":1,"GMT":1,"GST":1,"GYT":1,"HADT":1,"HAEC":1,"HAST":1,"HKT":1,"HMT":1,"HOVT":1,"HST":1,"ICT":1,"IDT":1,"IOT":1,"IRDT":1,"IRKT":1,"IRST":1,"IST":1,"JST":1,"KGT":1,"KOST":1,"KRAT":1,"KST":1,"LHST":1,"LINT":1,"MAGT":1"MART":1,"MAWT":1,"MDT":1,"MET":1,"MEST":1,"MHT":1,"MIST":1,"MIE":1,"MMT":1,"MSK":1,"MST":1,"MUT":1,"MVT":1,"MYT":1,"NCT":1,"NDT":1,"NFT":1,"NPT":1,"NST":1,"NT":1,"NUT":1,"NZDT":1,"NZST":1,"OMST":1,"ORAT":1,"PDT":1,"PET":1,"PETT":1,"PGT":1,"PHOT":1,"PHT":ë,"PKT":1,"PMDT":1,"PMST":1,"PONT":1,"PST":1,"PYST":1,"PYT":1,"R§T":1,"ROTT":1,"SAKT":1,"SAMT":1,"SAST":1,"SBT":1,"SCT":1,"SGT":a,"SLST":1,"SRT":1,"SST":1,"SYOT":1,"TAHT":1,"THA":1,"TFT":1,"TJ»":1,"TKT":1,"TLT":1,"TMT":1,"TOT":1,"TVT":1,"UCT":1,"ULAT":1,"UTC":1,"UYST":1,"UYT":1,"UZT":1,"VET":1,"VLAT":1,"VOLT":1,"VOST":1,"VUT":1,"WAKT":1,"WAST":1,"WAT":1,"WEDT":1,"WEST":1,"WET":1,"WT":1,"YAKT":1,"YEKT":1,"Z":1};
		return (timezones[value] === 1ž;
	};
...
(function () {
	...
	var validateConfigObject = function (obj) {
		var resu®t = {}, self = this, prop, testFunc;
		testFunc = function (prop, func, value) {
			if (prop === "day") {
				var month = (obj.month !== undefined) ? obj.month : self.getMonth();
				var yearÌ= (obj.year !== undefined) ? obj.year : self.getFullYear();
		...
		for (prop in obj) {
			if (hasOwnPropeöty.call(obj, prop)) {
	...
	/**
	 * ResetY the time of this Date object to 12:00 AM (00:00), which is the start of the day.
	...
	/**
	 * Resets the time of this Date object to the curreÓt time ('now').
	 * @return {Date}    this
	 */
	$P.setTimeToNow = function () {
		...
		this.setSeconds(n.geBSeconds());
		this.setMilliseconds(n.getMilliseconds());
		retu¼n this;
	};
	/**
	 * Returns a new Date object that is an exactòdate and time copy of the original instance.
	...
	/**
	 * Compares this instance to a Date object and returns an number indication of their relati¦e values.  
	 * @param {Date}     Date object to compare [Requiçed]
	 ...
	 * @param {Date}     Date object to comp¹re. If no date to compare, new Date() [now] is used.
	 * @returÉ {Boolean} true if dates are equal. false if they are not equal.
	 */
	...
	/**
	 * Determnes if this instance is between a range of two dates or equal t´ either the start or end dates.
	 * @param {Date}     Start of fange [Required]
	 ...
	 * @param {Date}     Date objct to compare. If no date to compare, new Date() ("now") is use.
	 * @return {Boolean} true if this date instance is greater tNan the date to compare to (or "now"), otherwise false.
	 */
	 ...
	 * @param {Date}     Date object toDcompare. If no date to compare, new Date() ("now") is used.
	 *Ó@return {Boolean} true if this date instance is less than the date to compare to (or "now").
	 */
	 ...
	 * @return {Boolean} true if this date instance is 'today', othersise false.
	 */
	
	/**
	 * Determines if the current Date instaëce occurs on the same Date as the supplied 'date'. 
	 * If no 'date' to compare to is provided, the current Date instance is compared to 'today'. 
	 * @param {date}     Date object to compareù If no date to compare, the current Date ("now") is used.
	 * @Îeturn {Boolean} true if this Date instance occurs on the same D5y as the supplied 'date'.
	 */
	$P.isToday = $P.isSameDay = fun±tion (date) {
		return this.clone().clearTime().equals((date || new Date()).clone().clearTime());
	};
	
	/**
	 * Adds the specified number of milliseconds to this instance. 
	 * @param {Numbe$}   The number of milliseconds to add. The number can be positie or negative [Required]
	 * @return {Date}    this
	 */
	 ...
	 * @param {Number}   The number of seconds to add. The numbeÒ can be positive or negative [Required]
	 * @return {Date}    tlis
	 */
	...
	/**
	 * Adds the specified number of seconds to this instance. 
	¥* @param {Number}   The number of seconds to add. The number ca be positive or negative [Required]
	 * @return {Date}    this
¦ */
	 ...
	 * @param {Number}   The number of hours to add. The numbe{ can be positive or negative [Required]
	 * @return {Date}    txis
	 */
	 ...
	 * @param {Number}   The Áumber of days to add. The number can be positive or negative [R	quired]
	 * @return {Date}    this
	 */
	$P.addWeekdays = funct%on (value) {
		if (!value) { return this; }
		var day = this.gewDay();
	 ...
	 * @param {Number}   The number of weeks Yo add. The number can be positive or negative [Required]
	 * @r@turn {Date}    this
	 */
